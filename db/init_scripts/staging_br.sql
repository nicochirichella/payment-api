...
BEGIN
	...
	VALUES ('ec_br', 'f6981d1b-5619-456f-ab29-3d17c4e85026', 'https://staging.trocafone.com/notifications/ipn')
    ...
    INSERT INTO gateway_methods (tenant_id, type, name, enabled, created_at, updated_at, post_execute_ttl, payment_ttl_include_weekends, pre_execute_ttl, ui_url, payment_method_id) VALUES (tenantId, 'MERCADOPAGO_CC', 'Credit Card MP', true, now(), now(), 1440, true, null, 'https://s3.amazonaws.com/payment-frontend.trocafone.com/multiple-credit-cards-stg/index.html?v=3&tenant=ec_br&paymentMethod=TWO_CREDIT_CARDS&numberofpayments=2&environment=staging&formatter=mercadopago', null);
    INSERT INTO gateway_methods (tenant_id, type, name, enabled, created_at, updated_at, post_execute_ttl, payment_ttl_include_weekends, pre_execute_ttl, ui_url, payment_method_id) VALUES (tenantId, 'MERCADOPAGO_TICKET', 'Ticket', true, now(), now(), 10080, true, null, 'https://s3.amazonaws.com/payment-frontend.trocafone.com/ticket-stg/index.html?paymentMethod=TICKET&tenant=ec_br&environment=staging&language=pt', null) RETURNING id INTO mercadopago_ticket_GatewayMethodId;
    INSERT INTO gateway_methods (tenant_id, type, name, enabled, created_at, updated_at, post_execute_ttl, payment_ttl_include_weekends, pre_execute_ttl, ui_url, payment_method_id) VALUES (tenantId, 'PAYPAL', 'Paypal', true, now(), now(), 4320, true, 30, 'https://s3.amazonaws.com/payment-frontend.trocafone.com/paypal/index.html?paymentMethod=PAYPAL&tenant=ec_br&environment=staging&language=pt&v=3', null) RETURNING id INTO paypal_GatewayMethodId;
